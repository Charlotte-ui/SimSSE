...
export class PlastronService {
  ...
  constructor(public apiService:ApiService, public modeleService:ModeleService, public profilService:ProfilService) { }
...
//    let HemoCue = new VariablePhysioTemplate({"3",1,"HemoCue",0,100,"#5470c5",36})
 ...
 //   let PAS = new VariablePhysioTemplate({"5",1,"PAS",0,100,"#5470c5",130})
   ...
   * @param plastron 
   */
  createPlastron(plastron: Plastron,idGroupe:string,idModele:string):Observable<any>{
    ...
    requests.push(this.apiService.createDocument(plastron)
    .pipe(map(response => this.apiService.documentId(response))))

    requests.push(this.profilService.createProfil(new Profil({age:randomAge})))

    return forkJoin(requests).pipe(switchMap((response:[string,Profil]) =>{
      console.log("createPlastron")
      let profil = response[1];
      let idPlastron = response[0]
      let idProfil = profil.id

      return this.apiService.createRelationBetween(idProfil,idPlastron,"aProfil")
      .pipe(map(response => response.result[0].out))
      .pipe(switchMap(()=>{
        return this.apiService.createRelationBetween(idPlastron,idGroupe,'seComposeDe')
        .pipe(switchMap(() => {
          return this.apiService.createRelationBetween(idModele,idPlastron,'aModele')
